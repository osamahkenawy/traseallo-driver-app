...
const packagesApi = {
  ...
  getOrderPackages: (orderId) =>
    apiClient.get(`/packages/order/${orderId}`),
   ...
   */
  scanLookup: (barcode) =>
    apiClient.get(`/packages/scan/${encodeURIComponent(barcode)}`),
  ...
  logScan: (packageId, data) =>
    apiClient.post(`/packages/${packageId}/scan`, data),
  ...
  updateStatus: (packageId, data) =>
    apiClient.patch(`/packages/${packageId}/status`, data),
};
